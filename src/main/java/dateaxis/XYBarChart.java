...
public class XYBarChart<X, Y> extends XYChart<X, Y> {
    ...
    if (shouldAnimate()) {
      ...
      dataRemoveTimeline.setOnFinished(new EventHandler<ActionEvent>() {
        public void handle(ActionEvent event) {
          ReflectionUtils.forceMethodCall(Data.class, "setSeries", item,
              new Object[] { null }); // TODO: make sure this is working as expected
          getPlotChildren().remove(bar);
        ...
        if (item != null) {
          ...
          if (categoriesOnScreen != null && categoriesOnScreen.contains(category)) {
            if (orientation == Orientation.VERTICAL) {
              ...
              valPos = getYAxis()
                  .getDisplayPosition((Y) ReflectionUtils.forceMethodCall(Data.class, "getCurrentY", item));
            }
            else {
              categoryPos = getYAxis()
                  .getDisplayPosition((Y) ReflectionUtils.forceMethodCall(Data.class, "getCurrentY", item));
              valPos = getXAxis()
                  .getDisplayPosition((X) ReflectionUtils.forceMethodCall(Data.class, "getCurrentX", item));
            }
  ...
  private double calculateCategorySize(SortedSet categories) {
    ...
    halfCategorySize = Math
        .min(xAxis.getDisplayPosition(xAxis.getUpperBound()) - xAxis.getDisplayPosition((long) categories.last()),
            halfCategorySize);
  ...
  private void animateDataAdd(Data<X, Y> item, Node bar) {
    ...
    if (orientation == Orientation.VERTICAL) {
      ...
      ReflectionUtils.forceMethodCall(Data.class, "setCurrentY", item, new Class[] { Object.class },
      ...
      ReflectionUtils.forceMethodCall(Chart.class, "animate", this, new Object[] { new KeyFrame[] {
          new KeyFrame(Duration.ZERO, new KeyValue(/*item.currentYProperty(),*/
              (ObjectProperty<Y>) ReflectionUtils.forceMethodCall(Data.class, "currentYProperty", item),
      ...
      ReflectionUtils.forceMethodCall(Data.class, "setCurrentX", item, new Class[] { Object.class },
      ...
      ReflectionUtils.forceMethodCall(Chart.class, "animate", this,
          new KeyFrame(Duration.ZERO, new KeyValue(/*item.currentXProperty(),*/
              (ObjectProperty<X>) ReflectionUtils.forceMethodCall(Data.class, "currentXProperty", item),
  ...
  private Timeline createDataRemoveTimeline(final Data<X, Y> item, final Node bar, final Series<X, Y> series) {
    ...
    if (orientation == Orientation.VERTICAL) {
      ...
      t.getKeyFrames().addAll(new KeyFrame(Duration.ZERO,
              new KeyValue(/*item.currentYProperty()*/(ObjectProperty<Y>) ReflectionUtils
                  .forceMethodCall(Data.class, "currentYProperty", item), /*item.getCurrentY()*/
      ...
      t.getKeyFrames().addAll(new KeyFrame(Duration.ZERO, new KeyValue(/*item.currentXProperty()*/
              (ObjectProperty<X>) ReflectionUtils
                  .forceMethodCall(Data.class, "currentXProperty", item), /*item.getCurrentX()*/
  ...
  private void updateDefaultColorIndex(final Series<X, Y> series) {
    ...
    Map<Series, Integer> seriesColorMapValue = (Map<Series, Integer>) ReflectionUtils
        .forceFieldCall(XYChart.class, "seriesColorMap", this);
  ...
  private Node createBar(Series series, int seriesIndex, final Data item, int itemIndex) {
    ...
    String defaultColorStyleClassValue = (String) ReflectionUtils
        .forceFieldCall(Series.class, "defaultColorStyleClass", series);
    bar.getStyleClass()
        .addAll("chart-bar", "series" + seriesIndex, "data" + itemIndex, /*series.defaultColorStyleClass*/
